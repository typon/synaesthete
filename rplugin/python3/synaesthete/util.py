...
def make_logger():
    logger = logging.getLogger('synaesthete')
    logger.setLevel(logging.ERROR)
    log_file = os.environ.get('SYNAESTHETE_LOG_FILE')
    if log_file:
        handler = logging.FileHandler(log_file)
        logger.setLevel(os.environ.get('SYNAESTHETE_LOG_LEVEL', logging.ERROR))
        logger.addHandler(handler)
    logger.debug('Synaesthete logger started.')
    return logger
